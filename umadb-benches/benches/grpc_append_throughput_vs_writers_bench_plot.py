...
# Keep this in sync with benches/grpc_append_throughput_vs_writers_bench.rs
...
bench_group = f"grpc_append_throughput_vs_writers{_with_docker}"
...
for t in threads:
    ...
    events_per_iter = 1  # single event per iteration per client
...
if len(x) == 0:
    print(f"Error: No benchmark data found for grpc_append_throughput_vs_writers")
    print(f"Expected to find data in: target/criterion/grpc_append_throughput_vs_writers{_with_docker}/*/new/")
...
plt.tight_layout()
plt.savefig(f"images/UmaDB-append-throughput-vs-writers-bench{_with_docker.replace('_', '-')}.png", format="png", dpi=300)
plt.show()
