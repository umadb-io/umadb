...
bench_group = f"grpc_read_cond{_with_docker}"
...
plt.title(f'UmaDB: Conditional Read Operations{' With Docker' if _with_docker else ''} ({EVENTS_PER_TAG} events per tag)')
...
plt.tight_layout()
plt.savefig(f"images/UmaDB-read-cond-bench{_with_docker.replace('_', '-')}.png", format="png", dpi=300)
plt.show()
