...
pub struct AsyncUmaDBClient {
    client: umadb_proto::UmaDbServiceClient<Channel>,
    batch_size: Option<u32>,
}
...
impl AsyncUmaDBClient {
    ...
    pub async fn connect_with_tls_options(
        ...
        match new_channel(url, tls_options).await {
            Ok(channel) => Ok(Self {
                client: umadb_proto::UmaDbServiceClient::new(channel),
                batch_size,
            }),
...
impl DCBEventStoreAsync for AsyncUmaDBClient {
    ...
    async fn read<'a>(
        ...
        let request = umadb_proto::ReadRequest {
            query: query_proto,
            start,
        ...
        let response = client.read(request).await.map_err(umadb_proto::dcb_error_from_status)?;
    ...
    async fn head(&self) -> DCBResult<Option<u64>> {
        let mut client = self.client.clone();
        match client.head(umadb_proto::HeadRequest {}).await {
    ...
    async fn append(
        ...
        let events_proto: Vec<umadb_proto::Event> = events.into_iter().map(umadb_proto::Event::from).collect();
        let condition_proto = condition.map(|c| umadb_proto::AppendCondition {
            fail_if_events_match: Some(c.fail_if_events_match.into()),
            after: c.after,
        });
        let request = umadb_proto::AppendRequest {
            events: events_proto,
...
pub struct AsyncClientReadResponse {
    stream: tonic::Streaming<umadb_proto::ReadResponse>,
...
impl AsyncClientReadResponse {
    pub fn new(stream: tonic::Streaming<umadb_proto::ReadResponse>) -> Self {
        Self {
            stream,
