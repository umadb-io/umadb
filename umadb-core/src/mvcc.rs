...
pub struct Writer {
    ...
    pub freed_page_ids: FxHashSet<PageID>,
...
impl Writer {
    pub fn new(
        ...
        Self {
            ...
            freed_page_ids: FxHashSet::default(),
    ...
    pub fn get_dirty_page_id(&mut self, page_id: PageID) -> DcbResult<PageID> {
        ...
        // 3. CoW Path: The page is clean, so we must copy it to a new PageID.
        self.freed_page_ids.insert(page_id);

        let new_page_id = self.alloc_page_id();
        let mut new_page =
            Arc::unwrap_or_clone(self.deserialized.remove(&page_id).ok_or_else(|| {
                DcbError::DatabaseCorrupted(format!(
                    "Deserialized page {page_id:?} not found while marking dirty"
                ))
            })?);
    ...
    pub fn append_freed_page_id(&mut self, page_id: PageID) {
        if self.freed_page_ids.insert(page_id) {
    ...
    pub fn process_freed_page_ids<T: MvccSnapshot>(
        ...
        while !self.reused_page_ids.is_empty() || !self.freed_page_ids.is_empty() {
            ...
            // Process freed page IDs by taking ownership of the current batch.
            // This leaves self.freed_page_ids empty and ready to receive any newly
            // freed pages generated by the B-tree inserts below.
            let freed_page_ids = std::mem::take(&mut self.freed_page_ids);

            for freed_page_id in freed_page_ids {
...
mod tests {
    ...
    fn test_copy_on_write_page_reuse() {
        ...
        // Second transaction
        {
            ...
            // Check the freed page IDs
            assert_eq!(1, writer.freed_page_ids.len());
            assert!(writer.freed_page_ids.contains(&PageID(6)));
